...
# --------------------------------------------------
# Glue Service Role (já existia – mantém)
# --------------------------------------------------
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:lambda:${var.region}:${data.aws_caller_identity.current.account_id}:function:export_crypto_data_to_s3"  # ← ADICIONAR ESTA LINHA
        ]
      }
    ]
  })
